...
<template>
    ...
    <section class="py-4 bg-slate-400 overflow-hidden flex flex-col gap-2 md:gap-4 2xl:gap-6">
        ...
        <div class="flex w-max hover:[animation-play-state:paused] animate-[scroll-l_120s_linear_infinite]">
            <div v-for="(img, idx) in duplicatedImages" :key="'r1-' + idx" class="flex-none px-1 md:px-2 2xl:px-3">
                <img :src="img" class="rounded-md bg-white w-40 md:w-60 lg:w-80 2xl:w-100 aspect-video" />
            </div>
        </div>

        <div class="flex w-max hover:[animation-play-state:paused] animate-[scroll-r_120s_linear_infinite]">
            <div v-for="(img, idx) in duplicatedImages" :key="'r2-' + idx" class="flex-none px-1 md:px-2 2xl:px-3">
                <img :src="img" class="rounded-md bg-white w-40 md:w-60 lg:w-80 2xl:w-100 aspect-video" />
            </div>
        </div>
